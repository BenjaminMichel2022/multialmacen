...
namespace pruebasAlmacen
{
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            //obtiene el almacen con mas productos
            var topGrupo = top.GroupBy(x => x);
            var topLargest = topGrupo.OrderByDescending(x => x.Count()).First();
            string almacenPrimario = topLargest.Key;

            Console.Write("El {0} es el que mas producto tiene", almacenPrimario);

            var banderaSecundaria = false;

            var almacenFinal = almacenPrimario;
            ...
            restart1:
                foreach (var element in list)
                {
                    ...
                    var CantidadEnvio = from env in cantidad
                                        ...
                                        select env;
                    if (list.Count > 1 || almacenFinal.Equals(element.Almacen))
                    {
                        //Almacen con mas producto de envio
                        if (almacenInicial.Almacen.Equals(element.Almacen) && almacenPrimario.Equals(element.Almacen))
                        {
                            envios.Add(new ProductoEnvio
                            {
                                ...
                                Observacion = "Almacen asignado al cliente con existencia para un solo envio"
                            });
                            contadorPrimario++;
                            almacenFinal = element.Almacen;
                            indice = 0;
                            break;
                        }
                    ...
                    restart:
                        foreach (var element in list)
                        {
                            if (producto.Descripcion.Equals(element.Producto))
                            {
                                ...
                                {
                                    //var suma = list.Sum(item => item.Cantidad);
                                    //if (!((suma - element.Cantidad) >= producto.Cantidad))
                                    /////////////////////////////////////////////////////////////
